<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            ...
            <?php echo form_open(site_url($this->config->item("user_update", "routes_uri") . "/" . $users_id)); ?>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-blue">
                    ...
                    <tbody> 
                        <tr>
                            <td>Name</td>
                            <td><input type="text" class="form-control" name="person_name" value="<?php echo set_value("person_name", $current_user["person_name"]); ?>" placeholder="Person Name" /></td>
                        </tr>
                        <tr>
                            <td>Login Username</td>
                            <td><input type="text" class="form-control" placeholder="Unique Username" name="username" value="<?php echo set_value("username", $current_user["username"]); ?>" /></td>
                        </tr>
                        <tr>
                            <td>Password</td>
                            <td><input type="password" class="form-control" placeholder="Password. Leave blank if no changes." name="users_password" value="<?php echo set_value("users_password"); ?>" /></td>
                        </tr>
                        <tr>
                            <td>Email</td>
                            <td><input type="email" class="form-control" placeholder="Email" name="email" value="<?php echo set_value("email", $current_user["email"]); ?>" /></td>
                        </tr>
                        <tr>
                            <td>API Key</td>
                            <td><input type="text" class="form-control" placeholder="API Key. Type in any words to regenerate API Key. Leave blank if no changes" name="api_key" value="<?php echo set_value("api_key"); ?>" /></td>
                        </tr>
                        <tr>
                            <td>Status</td>
                            <td>
                                <?php echo form_dropdown('status', array("1" => "Active", "0" => "Disabled"), set_value("status", $current_user["status"]), 'class="form-control"'); ?>
                            </td>
                        </tr>
                        <tr>
                            <td>Last Login</td>
                    ...
                    <tbody> 
                        <?php foreach($current_roles as $key=>$current_role){ ?>
                            <tr>
                                ...
                                <td>
                                    <?php if($this->matrix->checkSingleAccess($this->config->item("user_update", "routes_uri"), "*")){?>    
                                    <button type="button" class="btn btn-danger btn-sm delete-button" data-uri="<?php echo site_url($this->config->item("user_role_remove", "routes_uri") . "/" . $current_role["users_roles_id"]); ?>" data-role="<?php echo htmlspecialchars($current_role["roles_name"], ENT_QUOTES); ?>"><i class="fa fa-times"></i> Delete Role</button>
                                    <?php } ?>
                                </td>
                            </tr>
                        <?php } ?>
