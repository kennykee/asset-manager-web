<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            ...
            <h1 class="department-header"><i class="fa fa-cubes"></i> Import Asset From Excel</h1>
            
            <?php if(!$file_id){ ?>
                
            <div class="bs-callout">
                <h4>Upload An Excel File</h4>
                <p>1) Ensure the excel file is based on valid asset template.</p>
                <p>2) Ensure there are no empty rows in-between in the excel.</p>
                <p>3) Split into multiple files if there are more than 1000 rows.</p>
            </div>
            
            <div style="margin-top:5px;"><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#upload-modal"><i class="fa fa-upload"></i> Upload Excel File</a></div>
            
            <?php }else if(!$excel_row_data){ ?>
                
            <div class="bs-callout bs-callout-red">
                <h4>No More Data</h4>
                <p>There is no data in the file or you have reached end of the file. Ensure that there are no empty rows in-between in the excel.</p>
            </div> 
                
            <?php }else{ ?>
            
            <?php echo form_open(site_url($this->config->item("asset_import", "routes_uri") . "/" . $file_id . "?row_no=" . (intval($row_no)))); ?>
            
            <div class="clearfix button-height">
                <div class="pull-left small-margin-right"><button type="submit" class="btn btn-success"><i class="fa fa-check"></i> Save This Asset</a></div>
                    <div class="pull-right small-margin-right">
                        <h4 class="no-margin-bottom small-margin-top">
                            <i class="fa fa-hand-o-right"></i> Currently showing row: <?php echo $row_no; ?>
                        </h4>
                    </div>
                <div class="pull-left small-margin-right">
                    <a class="btn btn-primary" href="<?php echo site_url($this->config->item("asset_import_preview", "routes_uri") . "/" . $file_id . "?row_no=" . (intval($row_no) + 1)); ?>">
                        <i class="fa fa-chevron-circle-right fa-fw"></i> Skip To Next Row
                    </a>
                </div>
                
                <?php if($prev_id){ ?>
                <div class="pull-left">
                    <a class="btn btn-danger" target="_blank" href="<?php echo site_url($this->config->item("asset_individual_view", "routes_uri") . "/" . $prev_id); ?>">
                        <i class="fa fa-cubes fa-fw"></i> View Last Added Asset
                    </a>
                </div>
                <?php } ?>
                            ...
                            <td>
                                ...
                                <?php foreach($department_post as $dep){ ?>
                                    <div class="dropdown-margin-padding">
                                        ...
                                        <div class="col-md-4">
                                            <input type="text" class="form-control" name="locations[]" value="<?php echo set_value("locations[]", current(each($current_data["locations"]))); ?>" placeholder="Location" />
                                        </div>
                                        <div class="col-md-3">
                                            <input type="text" class="form-control" name="quantity[]" value="<?php echo set_value("quantity[]", current(each($current_data["quantity"]))); ?>" placeholder="Quantity" />
                                        </div>   
...
<script>
    ...
    <?php }else{ ?>
        operation_queue.push("init_asset_import");    
    <?php } ?>
</script>